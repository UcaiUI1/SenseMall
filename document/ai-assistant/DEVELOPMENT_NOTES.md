...
## 十五、mall-search 定时增量同步（已完成）

### 背景

此前 ES 索引与商品向量只在手动调用 `importAll/create` 时更新，商品改价/改名/下架后
语义搜索会持续返回旧数据。`pms_product` 无 `update_time` 字段，无法按时间戳增量，
故采用**内容哈希对比**方案。

### 实现

- `EsProduct` 增加 `contentHash`（内容 SHA-256，覆盖名称/副标题/关键词/价格/图片/品牌等展示字段）；
- `syncIncremental()`：对比 DB 商品哈希与 ES 存量哈希，仅重建变化商品的向量（避免全量重嵌入），
  并清理已下架/删除的文档（对比 DB 在售 id 集合）；
- 定时任务 `EsProductSyncTask`（默认 15 分钟，`mall.search.sync.interval-ms` 可配，可开关），
  另提供 `POST /esProduct/syncIncremental` 手动触发。

### 验证（实测通过）

- 改商品价格 → 同步仅更新该商品，ES 价格即时反映；
- 下架商品 → 同步自动清理 ES 文档（计数 20→19，GET 返回 404）；
- 恢复上架 → 同步自动重新入索引；
- 短间隔（20s）实测：改价后定时任务自动检测并同步，无需人工干预。
